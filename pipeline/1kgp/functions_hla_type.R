...
# Global variables
geneinfo            = fread("pipeline/hla_typing/gene_info.txt", sep = "\t", header = TRUE, data.table = FALSE)
id2hla              = fread("pipeline/hla_typing/id2url.txt"   , sep = "\t", header = TRUE, data.table = FALSE)
...
id2hla              = id2hla[ file.exists(id2hla$infile) == TRUE & !id2hla$id %in% c("HG00187", "HG01924", "HG01926", "HG01927", "NA19154"), ]
...
run_hla_typing = function(taskid, id2url)
{
    id      = id2url[taskid, "id"      ]
